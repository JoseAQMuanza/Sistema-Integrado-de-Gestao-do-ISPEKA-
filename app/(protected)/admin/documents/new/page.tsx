  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (usuario?.departamento) {
        setFormData(prev => ({ ...prev, departamento: usuario.departamento }));
      }
    };
    ...
    try {
      ...
      if (selectedUsers.length > 0) {
        ...
        if (shareError) {
          ...
          // Não interrompemos o processo por erro no compartilhamento
        }
      }
      ...
      const mensagemSucesso = `Documento "${formData.titulo}" criado com sucesso!\n\n` +
        ...
        (selectedUsers.length > 0 ? `\n\nCompartilhado com ${selectedUsers.length} usuário(s)` : '');
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="max-h-48 overflow-y-auto border border-gray-200 rounded-lg">
                        {usuariosCompartilhamento.map((usuario) => (
                          <label
                            key={usuario.id}
                          ...
                          >
                            ...
                            <div className="ml-3">
                              ...
                              <p className="text-xs text-gray-500">
                                {usuario.cargo?.nome} • {usuario.departamento?.nome}
                              </p>
                            </div>
                          </label>
                        ))}
                      </div>
                      {usuariosCompartilhamento.length === 0 && (
                        <p className="text-sm text-gray-500 text-center py-2">
                          Nenhum usuário disponível para compartilhamento
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-lg shadow p-6">
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <p className="text-sm text-gray-600">{userInfo?.cargo?.nome || 'Cargo não definido'}</p>
                      <p className="text-xs text-gray-500">
                        Departamento: {userInfo?.departamento || 'Não definido'}
                      </p>
                    </div>
                  </div>
                </div>
